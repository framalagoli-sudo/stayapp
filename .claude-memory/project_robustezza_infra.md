...
2. ~~**Verificare i cron Vercel**~~ ✅ FATTO (e trovato bug grave): i 4 cron (`/api/cron/*`: newsletter, automazioni, blog, backup R2) ERANO fermi — `vercel.json` li schedulava e i route verificano `Bearer ${CRON_SECRET}`, ma **`CRON_SECRET` non era impostato su Vercel** → Vercel cron riceveva 401 → backup notturno/newsletter/automazioni NON giravano da mesi (silent failure del passaggio serverless). FIX: impostato `CRON_SECRET` su Vercel (env Production) + redeploy. Verificato funzionalmente: cron con secret corretto → 200, sbagliato → 401. Lezione: ogni cron route che usa CRON_SECRET richiede la env impostata, altrimenti fallisce in silenzio.
