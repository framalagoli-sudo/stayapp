...
⛔ **Errore mio da non ripetere**: avevo messo la riga SOLO dentro le schede delle entità, che `isAdminAzienda` mostra al cliente — e Francesco, che entra da **super_admin**, in Dashboard non vedeva niente. È l'ultimo miglio applicato ai **ruoli**: una cosa è provata quando è aperta dal ruolo di chi la userà, e qui gli utilizzatori sono due. Ora c'è anche la sezione «Stato dei siti» per chi non è admin_azienda (fino a 12 righe).
