...
- `NEXT_PUBLIC_STAYAPP_DOMAIN` (valore `﻿oltrenova.com`, BOM in testa) → in `middleware.js` il confronto `hostname === STAYAPP_DOMAIN` falliva → `www.oltrenova.com` NON riconosciuto come dominio proprio → tutto il traffico cadeva nel ramo domini-custom. **Invisibile per le pagine normali** (il `next()` di fallback le serve comunque), ma `/en` → 404 (non è una route fisica, serve il rewrite attivo del ramo own-domain). Trovato 24/6 estraendo il valore inlinato dal bundle JS. Fix `.trim()` su tutte le letture: middleware + `DominiPage.jsx` (×2) + `app/api/domini/{route,[id]/route}.js` (var server `STAYAPP_DOMAIN`). Conferma: anche le env `NEXT_PUBLIC_*` inlinate nel client possono avere il BOM.
