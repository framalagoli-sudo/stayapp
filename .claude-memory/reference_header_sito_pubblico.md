...
- **Logo XOR nome (6/7)**: se c'è un logo l'header mostra SOLO il logo (nome nascosto); senza logo → nome testuale. `{navLogo ? <img> : <span nome>}`.
- **Dimensione logo (6/7)**: `minisito.logo_size` (jsonb, no migration) = small|medium|large → altezza header 24|32|48 (default medium). Selettore admin accanto al logo (Galleria struttura / Info ristorante-attivita); i 4 header applicano `logoH`.
- **Logo negativo (6/7)**: colonna `logo_dark_url` su properties/ristoranti/attivita (migration 064). Usato su sfondi scuri (footer scuro rilevato via `lib/color.js isDarkColor(bg)`; header quando `navDark`) con fallback a `logo_url`. Upload admin: "Logo per sfondi scuri" (Galleria struttura / Info ristorante-attivita) → endpoint logo con `?field=logo_dark_url` (path+colonna separati). guest-data + route [id] selezionano la colonna. ⚠️ La colonna va aggiunta con migration PRIMA del deploy (il select la referenzia).
