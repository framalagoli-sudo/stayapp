...
- **Fase 4 — Sito autonomo** ⏸️ IN ATTESA DECISIONE FRANCESCO (22/6). Analisi: l'obiettivo "sito non dipende dall'app" riguardava i CONTENUTI → **già raggiunto** da Fasi 1-3 (rich text, immagini proprie, galleria custom, pulsante, pattern). Restano: (1) **tema per-sito indipendente** = unico pezzo strutturale, ma trade-off: gli SMB di solito VOGLIONO stesso brand app+sito → rischio over-engineering per il target; (2) "Contenuti piattaforma" = già vero tecnicamente, lavoro solo cosmetico/etichette. **Proposta consigliata (A):** Fase 4 chiusa così + eventuale ritocco etichette "contenuti condivisi app+sito"; NON costruire tema separato salvo richiesta esplicita. **(B):** se Francesco vuole il tema per-sito, progettarlo a freddo (possibile senza migration nestandolo in `minisito.theme`, ma serve editor tema sito + renderer che preferiscono minisito.theme su theme entità).
