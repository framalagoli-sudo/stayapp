...
- **Fase 3 — Ibrido: correzione manuale:** ✅ FATTA 25/6. Tab **"Traduzioni"** in `MiniSitoPage` (editor sidebar) E `SitoPage` (componente `components/admin/TraduzioniSito.jsx`) + API `app/api/translations/[tipo]/[id]/route.js` (GET items IT→EN auto+override, PUT salva override; auth `requireEntityAccess`, 401 verificato). Campi non toccati = automatici (si ri-traducono); modificati = override fissi (vincono). "ripristina automatica" per campo + "rigenera automatiche" (azzera source_hash, tiene override). Ambito: contenuto entità (minisito/descrizione/servizi/menu) + **sotto-pagine** via selettore "Pagina" nell'editor (25/6): l'API `translations` gestisce `tipo='pagina'` (auth via entità proprietaria; `requireAuth` prima della lettura → no leak esistenza). FORM builder NON ancora nell'editor manuale (ma auto-tradotti). NB: editor traduzioni presente in BOTH MiniSitoPage (sidebar) e SitoPage. Helper `getTranslatableSource` estratto da `translate.js` (DRY). Verifica scrittura: solo interattiva (auth) — non testata da me per non sporcare il sito live di un cliente.
