...
- ✅ **Migration 127 eseguita e verificata** (11 FK in cascata, orfani a zero). `probe-cancella-azienda.mjs` in deploy.ps1: nessuna riga, file o account resta.
...
- ✅ **Categoria alla nascita LIVE** (25/09): le 3 route POST accettano `profilo` solo dal super_admin; `SceltaCategoria` obbligatoria in area Aziende e pagine elenco attività/ristoranti. Provato nel browser e in produzione. Le sonde ora puliscono dalla route (`cancellaAziendaDiProva`): 21 hostname orfani staccati da Vercel (restano 19 veri).
- ➡️ **PROSSIMO: onboarding «Inizia qui»** (STRATEGIA §4.1).
- ✅ «Parto da zero» rinominato **«Base»** (25/09, decisione di Francesco), chiave `da_zero` invariata, nessuna entità lo usa.
- Annotati 25/09: pagina elenco Strutture non crea per il super_admin (manca azienda_id); lo smoke security.spec cancella dal DB e può lasciare hostname su Vercel.
