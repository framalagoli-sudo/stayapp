...
## 🚪 DECISIONE 26/6: UN SOLO TOOL (no doppione)
Francesco: "voglio un solo tool non due, il cliente non sa dove andare. Template di pagina dentro la creazione a blocchi: SÌ". C'erano DUE porte AI per il sito: (1) voce sidebar "AI Site Builder" (top-level, `app/admin/ai-site-builder` + `AiSiteBuilderPage.jsx` + `/api/ai/generate-site` — wizard obiettivo+template-strutturali-astratti+preset → genera sito multi-pagina) e (2) galleria template dentro **Sito** (editor a blocchi, Fase A+B). → **Tenuto SOLO (2)**: la creazione/AI del sito vive nell'editor a blocchi "Sito". **Rimossa la voce sidebar "AI Site Builder"** (+ import Wand2) in AdminLayout. La pagina `ai-site-builder` resta orfana (URL diretto), NON linkata: serbatoio da cui FOLDARE nella galleria (Fase C) le parti buone = domande guidate (obiettivo/settore/tono/target) + generazione **multi-pagina** (oggi la galleria fa solo la home). NON ripristinare la voce in sidebar.
