...
import { getStruttura, getOfferta } from '@/lib/guest-data'
...
export async function generateMetadata(props) {
  ...
  const entita = await getStruttura(slug)
...
export default async function OffertaDetail(props) {
  ...
  let entity = entita
  if (lang === 'en') entity = await localizeEntity(entita, 'struttura', lang)
  return (
    <>
      <GuestSubPage entity={entity} entityType="struttura" pagina={costruisciPagina(offerta)} domain={searchParams?._domain || null} lang={lang} />
