...
export async function GET(request, { params }) {
  try {
    ...
    const { data, error } = await supabaseAdmin.from('form_builder').select('*')
...
export async function DELETE(request, { params }) {
  try {
    ...
    const { error } = await supabaseAdmin.from('form_builder').delete().eq('id', params.id).eq('azienda_id', profile.azienda_id)
    if (error) return Response.json({ error: error.message }, { status: 500 })
