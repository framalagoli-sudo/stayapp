...
export async function GET(request, { params }) {
  try {
    ...
    let q = supabaseAdmin.from('ristoranti').select('*').eq('id', params.id)
...
export async function PATCH(request, { params }) {
  try {
    ...
    if (body.slug !== undefined) {
      ...
      const { data: existing } = await supabaseAdmin.from('ristoranti')
    ...
    const profile = await getProfile(user.id)
    if (!profile) return Response.json({ error: 'Profilo non trovato' }, { status: 403 })
    let q = supabaseAdmin.from('ristoranti').update({ ...updates, updated_at: new Date().toISOString() }).eq('id', params.id)
    if (profile.role !== 'super_admin') q = q.eq('azienda_id', profile.azienda_id)
...
export async function DELETE(request, { params }) {
  try {
    ...
    let q = supabaseAdmin.from('ristoranti').delete().eq('id', params.id)
    if (profile.role !== 'super_admin') q = q.eq('azienda_id', profile.azienda_id)
    const { error } = await q
