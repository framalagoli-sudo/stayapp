...
export async function GET(request, props) {
  ...
  const { data, error } = await supabaseAdmin
    .from('entita')
    .select('id, azienda_id, tipo, slug, name, description, address, phone, whatsapp, wifi_name, wifi_password, checkin_time, checkout_time, rules, amenities, logo_url, cover_url, plan, moduli, theme, services, gallery, restaurant, activities, excursions, minisito, privacy_data, chatbot')
