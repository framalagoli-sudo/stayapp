...
export async function POST(request) {
  try {
    ...
    if (searchParams.get('property_id') && profile?.role !== 'super_admin') {
      const { data: prop } = await supabaseAdmin.from('entita').select('azienda_id').eq('id', propertyId).single()
    ...
    await supabaseAdmin.from('entita').update({ [field]: result.url }).eq('id', propertyId)
...
export async function DELETE(request) {
  try {
    ...
    const { data: prop } = await supabaseAdmin.from('entita').select(field).eq('id', profile.property_id).single()
    if (prop?.[field]) {
    ...
    await supabaseAdmin.from('entita').update({ [field]: null }).eq('id', profile.property_id)
