...
import { getAttivita, getOfferta } from '@/lib/guest-data'
...
export async function generateMetadata(props) {
  ...
  const entita = await getAttivita(slug)
...
export default async function OffertaDetail(props) {
  ...
  let entity = entita
  if (lang === 'en') entity = await localizeEntity(entita, 'attivita', lang)
  return (
    <>
      <GuestSubPage entity={entity} entityType="attivita" pagina={costruisciPagina(offerta)} domain={searchParams?._domain || null} lang={lang} />
