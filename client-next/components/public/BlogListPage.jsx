  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f9f9fb', fontFamily: "'Inter', sans-serif" }}>
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '60px 24px' }}>
        ...
        <p style={{ color: '#888', fontSize: 15, marginBottom: 48 }}>{t('news_subtitle', lang)}</p>

        {loading && <p style={{ color: '#aaa' }}>{lang === 'en' ? 'Loading…' : 'Caricamento…'}</p>}

        {!loading && articles.length === 0 && (
          <p style={{ color: '#aaa' }}>{lang === 'en' ? 'No articles published.' : 'Nessun articolo pubblicato.'}</p>
        )}

        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: 24 }}>
          {articles.map(art => (
            <a key={art.id} href={`${lp}/blog/${art.slug}`}
              ...
              <div style={{ padding: '20px 22px' }}>
                {art.published_at && (
                  <div style={{ fontSize: 11, color: '#aaa', marginBottom: 8, textTransform: 'uppercase', letterSpacing: 0.5 }}>
                    {new Date(art.published_at).toLocaleDateString(lang === 'en' ? 'en-GB' : 'it-IT', { day: '2-digit', month: 'long', year: 'numeric' })}
                  </div>
                )}
                ...
                <span style={{ fontSize: 13, fontWeight: 700, color: '#00b5b5' }}>{t('read_arrow', lang)}</span>
              </div>
            </a>
          ))}
        </div>
