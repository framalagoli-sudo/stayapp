...
import { formatoEuro, USD_PER_EUR } from '@/lib/valuta-ai'
        ...
        {!dati.ai ? (
          ...
          <>
            <div style={{ fontSize: 13, color: '#888', marginBottom: 14 }}>
              Mese UTC, costi calcolati dai token veri e convertiti al cambio fisso di 1 € = {String(USD_PER_EUR).replace('.', ',')} $.
              Tetto predefinito {formatoEuro(dati.ai.budgetPredefinito)} per azienda;
                ...
                return (
                  <div key={a.nome} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                    ...
                    <div style={{ width: 150, textAlign: 'right', fontSize: 13, color: '#555', fontVariantNumeric: 'tabular-nums' }}>
                      <strong>{formatoEuro(a.costo)}</strong>{a.budget ? ` / ${formatoEuro(a.budget)}` : ''} · {a.chiamate}
                    </div>
                  </div>
                )
              })}
