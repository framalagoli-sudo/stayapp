...
export default function PropertyInfoPage() {
  ...
  return (
    <div style={{ maxWidth: 640 }}>
      ...
      <CanaliSection property={property} save={save} />
    </div>
  )
}

function CanaliSection({ property, save }) {
  const [saving, setSaving] = useState(null)

  async function togglePwa(value) {
    setSaving('pwa')
    const mods = { ...(property.modules || {}), pwa_active: value }
  ...
  async function toggleMinisito(value) {
    setSaving('mini')
    const mini = { ...(property.minisito || {}), active: value }
    try { await save({ minisito: mini }) } catch {}
    setSaving(null)
  }

  const pwaOn  = property.modules?.pwa_active !== false
  const miniOn = !!property.minisito?.active

  return (
    <div style={{ background: '#fff', borderRadius: 12, padding: 28, boxShadow: '0 1px 4px rgba(0,0,0,0.06)', marginTop: 16 }}>
