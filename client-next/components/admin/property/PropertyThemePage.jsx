...
const DEFAULT_THEME = {
  primaryColor: '#00b5b5', secondaryColor: '', bgColor: '#ffffff', textColor: '#1a1a2e',
...
export default function PropertyThemePage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      const others = [
        ...(props || []).filter(p => p.id !== property.id).map(p => ({ ...p, _tipo: 'struttura' })),
        ...(rests || []).map(r => ({ ...r, _tipo: 'ristorante' })),
      ]
      setEntities(others)
    }).catch(() => {})
  }, [property?.id])
