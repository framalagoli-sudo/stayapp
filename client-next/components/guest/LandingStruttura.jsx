...
const DEFAULT_ORDER = [
  ...
  'testimonianze', 'recensioni', 'promozioni', 'pacchetti',
                ...
                {recensioni.slice(0, 6).map(r => (
                  <div key={r.id} style={{ background: '#f9f9fb', borderRadius: 16, padding: '24px 22px', display: 'flex', flexDirection: 'column', gap: 12 }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      ...
                      {FONTE_LABEL[r.fonte] && (
                        <span style={{ fontSize: 11, color: '#888', background: '#e8e8e8', padding: '2px 7px', borderRadius: 20 }}>
                          {FONTE_LABEL[r.fonte]}
                        </span>
                      )}
          ...
          <section key="booking" style={{ padding: '80px 0', background: '#f8f8f8' }}>
            <div className="land-section">
              <h2 style={{ fontFamily: heading, fontSize: 'clamp(22px, 3vw, 34px)', fontWeight: 700, marginBottom: 10, textAlign: 'center' }}>
                {mini.booking_section_title || 'Prenota un servizio'}
              </h2>
              ...
              <div style={{ display: 'flex', justifyContent: 'center' }}>
                <BookingWidget entityTipo="struttura" entityId={property.id} primaryColor={primary} />
              </div>
            </div>
          </section>
        )
  ...
  return (
    <>
      ...
      <footer style={{ background: '#111', padding: '28px 24px', textAlign: 'center' }}>
        ...
        <p style={{ fontSize: 12, color: '#555' }}>© {new Date().getFullYear()} {property.name} · Powered by OltreNova</p>
        {property.slug && (
...
function NewsletterForm({ aziendaId, primary, privacyUrl }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      setState(res.pending_confirmation ? 'pending' : res.duplicate ? 'duplicate' : 'success')
