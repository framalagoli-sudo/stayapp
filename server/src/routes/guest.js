...
// POST /api/guest/chat — chatbot AI pubblico (no auth)
router.post('/chat', async (req, res) => {
  try {
    const { entity_tipo, entity_id, messages } = req.body
    if (!entity_tipo || !entity_id || !Array.isArray(messages) || messages.length === 0) {
      return res.status(400).json({ error: 'entity_tipo, entity_id, messages obbligatori' })
    }
    ...
    const { data: entity, error } = await supabase.from(table)
      .select('name, description, address, phone, email, schedule, services, minisito')
      .eq('id', entity_id).single()
    if (error || !entity) return res.status(404).json({ error: 'Entità non trovata' })
...
- Tipo: ${entity_tipo}`
    ...
    if (!apiRes.ok) {
      console.error('[guest/chat] Anthropic error', apiRes.status)
    ...
    console.error('[guest/chat]', err.message)
    res.status(500).json({ error: 'Errore del servizio. Riprova tra qualche istante.' })
  }
})
