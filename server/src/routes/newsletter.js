...
function bodySemplice(c, primary) {
  return `
    ...
    <tr><td style="padding:36px 36px 40px">
      ...
      ${c.text ? `<p style="font-size:16px;color:#444;line-height:1.8;margin:0 0 8px;font-family:Arial,sans-serif;white-space:pre-wrap">${esc(c.text)}</p>` : ''}
      ${ctaButton(c.cta_text, c.cta_url, primary)}
    </td></tr>`
}

function bodyPromozione(c, primary) {
  const orig = parseFloat(c.price_original)
  const disc = parseFloat(c.price_discounted)
  ...
  return `
    ...
    <tr><td style="padding:36px 36px 40px">
      ...
      ${c.text ? `<p style="font-size:16px;color:#444;line-height:1.8;margin:0 0 8px;font-family:Arial,sans-serif;white-space:pre-wrap">${esc(c.text)}</p>` : ''}
      ${ctaButton(c.cta_text, c.cta_url, primary)}
      ${c.conditions ? `<p style="font-size:12px;color:#aaa;margin-top:20px;font-family:Arial,sans-serif;line-height:1.6;margin:20px 0 0">${esc(c.conditions)}</p>` : ''}
    </td></tr>`
}
...
export function buildNewsletterHtml({ entityName, entityLogo, primary = '#1a1a2e', template_id, content, unsubscribeUrl }) {
...
// GET /api/newsletter/archive/:entity_tipo/:entity_id — archivio pubblico (NO auth)
router.get('/archive/:entity_tipo/:entity_id', async (req, res) => {
  ...
  const { data, error } = await supabase.from('newsletters')
    .select('id, subject, sent_at, template_id, content, entity_tipo, entity_id')
...
router.get('/', requireAuth, async (req, res) => {
  ...
  let q = supabase.from('newsletters')
    .select('id, subject, template_id, status, sent_at, recipients_count, entity_tipo, entity_id, created_at, updated_at')
...
router.post('/', requireAuth, async (req, res) => {
  ...
  const { subject = '', template_id = 'semplice', content = {}, entity_tipo = 'struttura', entity_id = null } = req.body
  const { data, error } = await supabase.from('newsletters').insert({
    azienda_id, subject, template_id, content, entity_tipo, entity_id: entity_id || null, status: 'draft',
  }).select().single()
  if (error) return res.status(500).json({ error: error.message })
  res.status(201).json(data)
})

// PATCH /api/newsletter/:id — aggiorna bozza
router.patch('/:id', requireAuth, async (req, res) => {
  const allowed = ['subject', 'template_id', 'content', 'entity_tipo', 'entity_id']
...
// POST /api/newsletter/:id/duplicate — duplica
router.post('/:id/duplicate', requireAuth, async (req, res) => {
  ...
  const { data, error: e2 } = await supabase.from('newsletters').insert({
    ...
    subject: `${orig.subject} (copia)`, template_id: orig.template_id,
    content: orig.content, status: 'draft',
  }).select().single()
  if (e2) return res.status(500).json({ error: e2.message })
  res.status(201).json(data)
})

// POST /api/newsletter/:id/test — invio di test
router.post('/:id/test', requireAuth, async (req, res) => {
  ...
  const html = buildNewsletterHtml({
    ...
    template_id: nl.template_id, content: nl.content,
  ...
  try {
    ...
    await resend.emails.send({
      ...
      to: test_email,
      subject: `[TEST] ${nl.subject || '(senza oggetto)'}`,
      html,
    })
    res.json({ ok: true })
...
// POST /api/newsletter/:id/send — invio reale a tutti gli iscritti
router.post('/:id/send', requireAuth, async (req, res) => {
  const { data: nl, error } = await supabase.from('newsletters').select('*').eq('id', req.params.id).single()
  if (error || !nl) return res.status(404).json({ error: 'Non trovata' })
  if (nl.status === 'sent') return res.status(400).json({ error: 'Newsletter già inviata' })
  if (!nl.subject?.trim()) return res.status(400).json({ error: 'Oggetto obbligatorio prima di inviare' })
  ...
  if (!contacts?.length) return res.status(400).json({ error: 'Nessun iscritto trovato' })
    ...
    for (let i = 0; i < contacts.length; i += 50) {
      ...
      const emails = batch.map(c => ({
        ...
        html: buildNewsletterHtml({
          ...
          template_id: nl.template_id, content: nl.content,
          unsubscribeUrl: `${appUrl}/unsubscribe?token=${c.unsubscribe_token || 'na'}`,
        }),
      }))
  ...
  await supabase.from('newsletters').update({
    status: 'sent', sent_at: new Date().toISOString(), recipients_count: sent, updated_at: new Date().toISOString(),
  }).eq('id', nl.id)
