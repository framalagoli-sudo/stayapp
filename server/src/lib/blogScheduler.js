...
import { fetchUnsplashCover } from './unsplash.js'

// ─── Utilità data ─────────────────────────────────────────────────────────────

function calcNextRun(frequenza, ora, giornoSettimana, giornoMese, from = new Date()) {
  ...
  if (frequenza === 'settimanale') {
    d.setUTCHours(ora)
    // Trova il prossimo giorno della settimana (JS: 0=dom)
...
async function generateArticle(automazione) {
  ...
  // Argomento: ruota la lista, oppure lascia scegliere a Claude
  let topicLine = ''
  if (argList.length) {
...
<p>${parsed.excerpt || ''}</p>
<p><a href="${process.env.CLIENT_URL || 'https://stayapp-henna.vercel.app'}/admin/blog">Apri l'admin Blog →</a></p>`,
      })
    } catch (e) { console.error('[blogScheduler] notifica email:', e.message) }
  }
}

// ─── Runner principale (chiamato ogni ora) ────────────────────────────────────

export async function runBlogScheduler() {
  const now = new Date().toISOString()

  const { data: automazioni } = await supabase.from('blog_automazioni')
    .select('*')
    .eq('attiva', true)
    .lte('next_run_at', now)
  ...
  for (const auto of automazioni) {
    ...
    await supabase.from('blog_automazioni').update({
      last_run_at: now,
