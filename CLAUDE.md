...
- [ ] 🧭 **Preconfiguratori per mestiere** (via libera di Francesco 23/09, progetto in `STRATEGIA.md` §6.1) — catalogo unico delle funzioni (entità **e** azienda: oggi ~25 voci azienda non hanno interruttore e i menu sono tre scritti a mano), profili in DB applicati **come copia** alla nascita, menu dell'azienda = unione delle sue entità, area super_admin «Funzioni e profili» con matrice aziende × funzioni. Si parte da **F0+F1**, che non cambiano niente per i clienti. `tipo` ≠ `profilo` ≠ `settore`. Coincide in gran parte con l'onboarding qui sotto.
