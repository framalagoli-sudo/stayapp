...
> **🧭 Dove andiamo e perché** → `STRATEGIA.md` — il documento cardine fra Francesco ed Ettore: ruoli, fotografia coi numeri, le tre strade, cosa costruire in ordine con l'effort, cosa NON fare, idee in valutazione (preconfiguratori per mestiere, aggregatore locale) e decisioni aperte. **Leggerlo prima di proporre funzioni nuove.**
...
46. **💳 Il primo incasso vero: il dato c'era, non lo disegnava nessuno** (22/09/2026). Garage 22, evento di prova, Agnese paga 1 €. Webhook arrivato, riga `pagamento_stato: pagato`, soldi sul conto del cliente — **tutto giusto**. E Francesco: «nella piattaforma che succede? nulla». Aveva ragione: il pagamento era registrato in **tre** punti e non era visibile in **nessuno**.
    - ⛔ **La pagina dopo il pagamento chiedeva all'endpoint sbagliato.** `/checkout/successo` interrogava `/api/shop/public/esito`, che cerca solo negli **ordini del negozio**: prenotazioni ed eventi cadevano sempre nel ramo «non trovato», e quel ramo parlava lo stesso di «ordine». Chi aveva pagato un posto a una cena leggeva *«Il tuo ordine è stato registrato»*, senza il nome dell'evento, senza il nome del locale, **senza un link per tornare indietro**. Una pagina bianca di uno sconosciuto, subito dopo aver dato i soldi.
    - Ora **`lib/esito-pagamento.js`** cerca nei tre posti **nello stesso ordine del webhook**: è la stessa domanda fatta da due lati, e due risposte diverse sarebbero un modo per divergere in silenzio. La pagina si veste del cliente (logo, colore) e in fondo ha la porta verso il suo **indirizzo ufficiale** (`hostUfficiale`, non il nostro percorso interno).
    - **`components/admin/StatoPagamento.jsx`**: un vocabolario solo per eventi, prenotazioni risorse e vista unificata. Esisteva già, ma **solo dentro `ShopPage`** — una scala di stati scritta in un componente è una scala che gli altri due punti in cui si incassa non hanno. ⚠️ `non_richiesto` **non** vuol dire «non pagato»: con un importo è «paga sul posto», senza non si scrive niente (scrivere «Da pagare» su un evento gratuito è una richiesta inventata).
...
49. **🧭 Il menu per quello che fa il titolare, col sito in cima** (24/09/2026, approvato da Francesco). Aperto da titolare, «Sito web» era la **voce 27 di 43**, fuori dallo schermo: il menu seguiva com'è fatto il database (funzioni di azienda sopra, di entità sotto), non come lavora il cliente.
    - **`components/admin/menu-pannello.js` → `costruisciMenu()` è l'unica regola** di cosa compare, per ruolo: la usano la barra laterale vera **e** l'anteprima dei profili. Gruppi: Il tuo sito · Contenuti · Clienti · Promozione · Impostazioni (· Piattaforma). `VOCI` e `SEZIONI` scritte una volta; nomi in italiano. Il percorso in alto prende i nomi da lì (prima tre dizionari chiamavano la stessa pagina «Sito», «Sito web» e «Pagine CMS»).
    - Il menu **non si disegna finché azienda ed entità non sono caricate** e senza un ruolo noto: prima compariva intero e poi perdeva voci davanti al cliente.
    - Un'entità che **esiste** si vede, per ogni ruolo: per il titolare contava solo `aziende.moduli.<tipo>`, e il titolare di D.O. Giachini (interruttore spento, attività presente) non ha mai visto il suo sito nel menu.
    - Sonda `tests/probe-menu-pannello.mjs` + `menu-pannello.atteso.json`: fotografa il menu **dal DOM** in 14 scenari (ruoli × permessi) e lo confronta. Cambiare il menu = aggiornare la fotografia **dopo** aver verificato che la differenza sia solo quella voluta (confronto degli indirizzi raggiungibili per ruolo).

50. **🏷️ Le funzioni le decidiamo noi, per categoria dell'ENTITÀ** (24–25/09/2026, migration `128`–`129`). Decisione di Francesco: il cliente non accende funzioni («gli smanettoni mettono cose che non hanno senso»); la categoria sta sull'entità (Borgo del Lago: una struttura e due ristoranti, ognuno la sua). `STRATEGIA.md` §6.1.
    - **Profili di mestiere** in `profili_mestiere` (8 categorie; «Parto da zero» della migration 128 è stato rinominato **«Base»** dall'editor il 25/09 — un ripristino dalle migration riporterebbe il nome vecchio), editor e anteprima in **Funzioni e profili** (solo super_admin); scheda **«Categorie»** per assegnarle; «Riapplica» porta un profilo migliorato alle entità della sua categoria. Un profilo in uso non si elimina (409).
    - **`lib/applica-profilo.js`**: il profilo si applica **come copia** negli interruttori dell'entità (`moduli`: in cima, annidati in `modules`, e sotto i nomi storici; **le altre chiavi — wifi, reception, allergens… — non si toccano**). `aziende.funzioni` = unione dei profili delle sue entità, **solo se tutte ne hanno uno** (altrimenti `null` = vede tutto).
    - ⛔ **Niente sparisce**: una funzione dell'entità con contenuti non si spegne; una funzione dell'azienda con righe nel database resta (`lib/uso-funzioni.js`, la stessa misura della pagina «Uso»). Senza questa seconda regola TVB avrebbe perso Piano editoriale, Automax le Offerte, Borgo del Lago il Blog.
    - **Alla nascita**: le route di creazione accettano `profilo` **solo dal super_admin** (403 al titolare), controllato prima di creare; `SceltaCategoria` è obbligatoria nei moduli dell'area Aziende e delle pagine elenco.
    - Sonde: `probe-categorie.mjs` (permessi, protezioni, nascita, menu nel browser), `probe-profili-mestiere.mjs`, `probe-funzioni-uso.mjs` (corpo grezzo: solo nomi e conteggi). **`probe-menu-clienti.mjs`** (a mano) apre il pannello di ogni cliente vero come il titolare: ⚠️ **con il 2FA fatto come una persona** — il 2FA dei clienti non si spegne mai per provare; la prima verifica senza secondo fattore mostrava un menu sbagliato per tutti, ed era la prova a essere sbagliata.

51. **🗑️ Cancellare porta via tutto — e una cancellazione filtrata non è un rifiuto** (24/09/2026, migration `127`).
    - 🔴 **Buco chiuso**: `DELETE /api/ristoranti|attivita/:id` di un'**altra** azienda cancellava 0 righe (filtro per azienda, nessun errore) e poi **staccava comunque i domini** dell'id ricevuto: un cliente poteva mandare offline il sito di un altro, e riceveva 200. La sweep non lo vedeva (guarda cosa esce, non cosa succede). Mai sfruttato (registro dal 14/05). Ora `cancellaEntita` (`lib/cancellazione.js`) chiede **«è sua?» prima di qualunque effetto**. Sonda `probe-cancella-entita-altrui.mjs` **in deploy.ps1**.
    - Cancellare un'azienda lasciava **gli account vivi e capaci di fare login** (`profiles.azienda_id` è `SET NULL`: così erano rimasti i collaboratori di Futura Vacanze), pagine, visite, file; con una prenotazione **non si poteva cancellare**. Ora: si **raccoglie** (sola lettura) → si cancella la **riga** (cascate della 127) → **solo allora** file, traduzioni e account. Se la riga non si cancella non si perde niente. La conferma nel pannello chiede di scrivere il nome. Sonda `probe-cancella-azienda.mjs` **in deploy.ps1**: cerca i resti in ogni tabella dello **schema pubblicato** da PostgREST, non in un elenco scritto a mano.
    - ⚠️ `entity_translations` **non** ha chiave esterna di proposito: il suo `entity_id` indica entità, pagine, eventi, articoli e form.

52. **🔐 Account senza azienda, `requireSuperAdmin`, cancello più intelligente** (24–25/09/2026).
    - Bloccati (ban reversibile, su ok di Francesco) i 7 account senza azienda: 6 di Futura Vacanze (cancellata il 17/09) e `oltrenova@gmail.com`. Il blocco rifiuta login **e** rinnovo della sessione.
    - `requireSuperAdmin` in `lib/server-auth.js` (404 a chi non lo è) al posto delle copie locali; `verifica-regole.mjs` lo riconosce.
...
- [x] 🧭 **Preconfiguratori per mestiere** ✅ 24–25/09/2026 — categorie per entità decise da noi, applicate a tutti i clienti e alla nascita; menu nuovo col sito in cima. Note 49–50. `tipo` ≠ `profilo` ≠ `settore`.
...
- **Gli eventi restano a parte: NON migrano dentro `offerte`** (27/08, riconfermato il 28/08 con la ragione vera). Voce di menu propria, tabella propria, prenotazioni proprie. Il motivo non è che i campi siano diversi — sono quasi identici — ma che **catalogo → offerte → shop sono consequenziali e l'evento no**: un evento non è una cosa che possiedi e che poi metti in offerta o in vendita, è un fatto che accade, con un inizio e una fine. Parole di Francesco: «l'evento è un'entità a sé rispetto a catalogo, offerte e shop, che sono consequenziali». Anche `event_bookings` resta fuori dall'unificazione delle prenotazioni.
