...
- **Vetrine — Fasi 1 + 2** ✅ 2026-07-08: motore generico "collezioni + elementi" (migration `065`, preset in `lib/vetrinePresets.js`, API + tab admin **Vetrine**). **Fase 1** = dati + admin (editor guidato dal preset). **Fase 2** = vetrina pubblica: blocco `vetrina` nel site-builder (griglia client-fetch via `/api/guest/vetrina/[id]`) + pagina dettaglio **SSR** `/{s|r|a}/[slug]/v/[itemSlug]` + sitemap. Primo preset **progetti immobiliari (flipping)**; numeri sensibili in `dati_privati` (gating verificato live: 0 leak). Nuovo verticale = nuovo preset, zero migration. Resta Fase 3 (lead gated). Vedi memoria `project_vetrine`.

### Da fare (in ordine)
- [ ] **Vetrine Fase 3** — lead gated: form "Voglio partecipare" → `requests`, numeri riservati recapitati dopo il contatto (oggi la CTA è un placeholder)
