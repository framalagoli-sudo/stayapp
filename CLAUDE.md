...
- **Vetrine (Fasi 1+2+3)** ✅ 2026-07-08: motore generico "collezioni + elementi" (migration `065`, preset in `lib/vetrinePresets.js`, API + tab admin **Vetrine**). **Fase 1** = dati + admin (editor guidato dal preset). **Fase 2** = vetrina pubblica: blocco `vetrina` (griglia via `/api/guest/vetrina/[id]`) + dettaglio **SSR** `/{s|r|a}/[slug]/v/[itemSlug]` + sitemap. **Fase 3** = form lead del dettaglio → `POST /api/guest/contact` (`source:'vetrina'`) → **CRM `contatti`** (tag `vetrina`, progetto in nota, notifica titolare). Preset **progetti immobiliari (flipping)** + **auto (autosalone nuovo/usato)**; `dati_privati` gated (0 leak verificato). Capability generiche: filtro a livello di blocco (una vetrina → pagine "nuove"/"usate"), CTA guidata dal preset, bottone WhatsApp per-elemento (precompilato). Nuovo verticale = nuovo preset (~20 righe), zero migration. Vedi memoria `project_vetrine`.
