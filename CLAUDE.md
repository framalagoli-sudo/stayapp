...
25. **🔐 Secondo fattore e passkey** —  è **attivo su tutte le aziende** e il default della colonna è  (migration 072): ogni azienda nuova nasce protetta. L'enforcement è in  () e vale per ogni route che passa da . Le **passkey sono un metodo di accesso**, non un fattore MFA: producono una sessione  con , che  accetta come autenticazione completa perché è legata al dominio e non rigiocabile da un sito civetta. Il client le abilita con . ⚠️ Se cambia il dominio del pannello, vanno aggiornati **Relying Party ID e Origins** su Supabase, altrimenti le passkey esistenti smettono di funzionare. Sonde: , , .
